...
{
    public class GiftOrderPatch
    {
        [HarmonyPatch(typeof(GiftAppearance), nameof(GiftAppearance.RefreshAppearance))]
        [HarmonyTranspiler]
        static IEnumerable<CodeInstruction> RefreshAppearanceTranspiler(IEnumerable<CodeInstruction> instructions, ILGenerator ilgen)
        {
            ...
            {
                if (!found && i < codes.Count - 8 && codes[i].Is(OpCodes.Ldc_I4, 1000) && codes[i + 1].IsStloc() && codes[i + 2].Is(OpCodes.Ldc_I4, 1000) && codes[i + 3].IsStloc()
                     && codes[i + 4].Is(OpCodes.Ldc_I4, 1000) && codes[i + 5].IsStloc() && codes[i + 6].Is(OpCodes.Ldc_I4, 1000) && codes[i + 7].IsStloc())
                {
                    found = true;
                    for (var j = 0; j < 4; j++)
                    {
                ...
                if (found && i < codes.Count - 1 && codes[i].IsLdloc() && codes[i + 1].Is(OpCodes.Ldc_I4_S, 100))
                {
